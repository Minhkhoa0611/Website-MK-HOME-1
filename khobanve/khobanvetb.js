document.addEventListener("DOMContentLoaded", function() {
    ...
    h4Element.innerHTML = `Các bạn yêu quý mình có thể đóng góp bản vẽ <a href="https://drive.google.com/drive/folders/1zgaeAdlBaxcKOQCO-ZAnFjHqH532W5B?usp=sharing" target="_blank" class="button">Tại đây</a>`;
    ...
    downloadButtons.forEach(button => {
        button.addEventListener("click", (event) => {
            ...
            const userConfirmed = confirm(
                "XÁC NHẬN CHỊU TRÁCH NHIỆM VỀ VẤN ĐỀ BẢO MẬT.\nCHÚNG TÔI XIN MIỄN TRỪ TRÁCH NHIỆM.\nBạn có đồng ý tiếp tục không?"
            );
            if (userConfirmed) {
                window.location.href = button.getAttribute("href"); // Redirect to the link
            }
        });
    });
});
